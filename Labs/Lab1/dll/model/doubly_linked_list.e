...
feature -- commands
	...
	add_after(n: NODE[G]; e: detachable G)
		...
		require
			node_exists (n)
		do
		    check attached n.next as nt then
					add_between (e, n, nt)
		end
		ensure
	...
	add (e: detachable G; i: INTEGER_64)
		...
		do
		    ...
		    l_node := node(i)
			check attached l_pred as lp
			  and attached l_node as ln
				then
					add_between (e, lp, ln)
				end
		    end
		    if not node_exists(node(i)) then
			l_pred := node(i).previous
			l_succ := node(i).next
			check attached l_succ as ls
				and attached l_pred as lp
	...
	remove_at (i: INTEGER)
		...
		do
			...
			count := count - 1
	     	...
	     	decrement_count: count = old count - 1
			element_removed_at_i: not node_exists ((old node(i)))
		end
