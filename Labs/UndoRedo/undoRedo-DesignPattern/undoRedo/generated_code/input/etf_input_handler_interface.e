...
feature -- parsing
	...
	evt_to_cmd (evt : TUPLE[name: STRING; args: ARRAY[ETF_EVT_ARG]]) : ETF_COMMAND_INTERFACE
		...
		do
			...
			elseif cmd_name ~ "move" then 
				 if attached {ETF_ENUM_INT_ARG} args[1] as a_direction and then (a_direction.value = left or else a_direction.value = right or else a_direction.value = up or else a_direction.value = down) then 
					 create {ETF_MOVE} Result.make ("move", [a_direction.value], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 

			elseif cmd_name ~ "jump" then 
				 if TRUE then 
					 create {ETF_JUMP} Result.make ("jump", [], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 
	...
	do
		...
		loop
			...
			elseif cmd_name ~ "move" then 
				if NOT( ( args.count = 1 ) AND THEN attached {ETF_ENUM_INT_ARG} args[1] as a_direction and then (a_direction.value = left or else a_direction.value = right or else a_direction.value = up or else a_direction.value = down)) then 
					if NOT Result.is_empty then
						Result.append ("%N")
					end
					Result.append (evt_out_str + " does not conform to declaration " +
							"move(a_direction: DIRECTION = {left, right, up, down})")
				end
